...
if [[ -z "$NO_LINUX" ]]; then
    echo 'Building linux binaries in docker. If this fails, try building with --no-cache and re-running (it will be a LOT slower)'
    docker build -t flow-bin .
    docker run -it  -v `pwd`/dist/linux:/root/flow/bin -e "FLOW_VERSION=$BRANCH" flow-bin:latest 
